...
namespace TeamCitySharp.ActionTypes
{
    public class BuildConfigs : IBuildConfigs
    {
        ...
        {
            BuildConfigs newInstance = (BuildConfigs)MemberwiseClone();
        ...
        {
            BuildConfig build = m_caller.GetFormat<BuildConfig>(ActionHelper.CreateFieldUrl("/app/rest/buildTypes/name:{0}", m_fields),
                                                       buildConfigName);
        ...
        {
            BuildConfig build = m_caller.GetFormat<BuildConfig>(ActionHelper.CreateFieldUrl("/app/rest/buildTypes/id:{0}", m_fields),
                                                       buildConfigId);
                                              ...
                                              string newBuildTypeId = "")
        {
            ...
            HttpResponse response = m_caller.Post(xmlData, HttpContentTypes.ApplicationXml,
        ...
        public BuildConfig Copy(string buildConfigId, string buildConfigName, string destinationProjectId,
                                string newBuildTypeId = "")
        {
            HttpResponse response = CopyBuildConfig(buildConfigId, buildConfigName, destinationProjectId, newBuildTypeId);
            if (response.StatusCode == HttpStatusCode.OK)
            {
                JsonReader reader =
                  new JsonReader(
                    new DataReaderSettings(new ConventionResolverStrategy(ConventionResolverStrategy.WordCasing.Lowercase, "-")));
                BuildConfig buildConfig = reader.Read<BuildConfig>(response.RawText);
                return buildConfig;
            }
        ...
        {
            HttpResponse response = CopyTemplateQuery(templateId, templateName, destinationProjectId, newTemplateId);
            if (response.StatusCode == HttpStatusCode.OK)
            {
                JsonReader reader =
                  new JsonReader(
                    new DataReaderSettings(new ConventionResolverStrategy(ConventionResolverStrategy.WordCasing.Lowercase, "-")));
                Template template = reader.Read<Template>(response.RawText);
                return template;
            }
            return new Template();
        }
        ...
        {
            string url = $"/app/rest/buildTypes/{locator}";
        ...
        {
            BuildConfig build = m_caller.GetFormat<BuildConfig>(ActionHelper.CreateFieldUrl("/app/rest/buildTypes/{0}", m_fields),
                                                       locator);
        ...
        public bool ModifTrigger(string buildTypeId, string triggerId, string newBt)
        {
            ...
            string urlExtractAllTriggersOld = $"/app/rest/buildTypes/id:{buildTypeId}/triggers";
            BuildTriggers triggers = m_caller.GetFormat<BuildTriggers>(urlExtractAllTriggersOld);
            foreach (BuildTrigger trigger in triggers.Trigger.OrderByDescending(m => m.Id))
            {
                ...
                foreach (Property property in trigger.Properties.Property)
                {
                    ...
                    string ttt = writer.Write(trigger);
                    string urlNewTrigger = $"/app/rest/buildTypes/id:{buildTypeId}/triggers";
                    HttpResponse response = m_caller.Post(ttt, HttpContentTypes.ApplicationJson, urlNewTrigger,
                                                HttpContentTypes.ApplicationJson);
                    if (response.StatusCode != HttpStatusCode.OK)
                    {
                        continue;
                    ...
                    string urlDeleteOld = $"/app/rest/buildTypes/id:{buildTypeId}/triggers/{trigger.Id}";
        ...
        {
            string urlExtractOld = $"/app/rest/buildTypes/id:{buildTypeId}/snapshot-dependencies/{dependencyId}";
            SnapshotDependency snapshot = m_caller.GetFormat<SnapshotDependency>(urlExtractOld);
            snapshot.Id = newBt;
            snapshot.SourceBuildType.Id = newBt;

            string urlNewTrigger = $"/app/rest/buildTypes/id:{buildTypeId}/snapshot-dependencies";
            JsonWriter writer =
              new JsonWriter(
                new DataWriterSettings(new JsonResolverStrategy()));

            string tmpArtifact = (writer.Write(snapshot));
            tmpArtifact = Regex.Replace(tmpArtifact, "source-build-type", "source-buildType");


            HttpResponse response = m_caller.Post(tmpArtifact, HttpContentTypes.ApplicationJson, urlNewTrigger, HttpContentTypes.ApplicationJson);
            if (response.StatusCode == HttpStatusCode.OK)
            {
                string urlDeleteOld = $"/app/rest/buildTypes/id:{buildTypeId}/snapshot-dependencies/{dependencyId}";
                m_caller.Delete(urlDeleteOld);
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    return true;
        ...
        {
            string urlAllExtractOld = $"/app/rest/buildTypes/id:{buildTypeId}/artifact-dependencies";
            ArtifactDependencies artifacts = m_caller.GetFormat<ArtifactDependencies>(urlAllExtractOld);
            foreach (ArtifactDependency artifact in artifacts.ArtifactDependency.OrderByDescending(m => m.Id))
            {
                if (dependencyId != artifact.SourceBuildType.Id)
                {
                    continue;
                }
                ...
                JsonWriter writer =
                  new JsonWriter(
                    new DataWriterSettings(new JsonResolverStrategy()));
                string tmpArtifact = writer.Write(artifact);
                tmpArtifact = Regex.Replace(tmpArtifact, "source-build-type", "source-buildType");

                string urlNewTrigger = $"/app/rest/buildTypes/id:{buildTypeId}/artifact-dependencies";

                HttpResponse response = m_caller.Post(tmpArtifact, HttpContentTypes.ApplicationJson, urlNewTrigger,
                                            HttpContentTypes.ApplicationJson);
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    string urlDeleteOld = $"/app/rest/buildTypes/id:{buildTypeId}/artifact-dependencies/{artifact.Id}";
                    m_caller.Delete(urlDeleteOld);
