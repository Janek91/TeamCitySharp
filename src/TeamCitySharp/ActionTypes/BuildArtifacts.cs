...
namespace TeamCitySharp.ActionTypes
{
    ...
    public class ArtifactWrapper
    {
        ...
        public ArtifactCollection Specification(string buildSpecification)
        {
            ...
            XmlNodeList artifactNodes = document.SelectNodes("//artifact");
            ...
            foreach (XmlNode node in artifactNodes)
            {
                ...
                XmlNode extensionNode = node.SelectSingleNode("@ext");
    ...
    public class ArtifactCollection
    {
        ...
        public List<string> Download(string directory = null, bool flatten = false, bool overwrite = true)
        {
            ...
            foreach (string url in m_urls)
            {
                ...
                string[] parts = url.Split('/').Skip(5).ToArray();
                string destination = flatten
                                    ? parts.Last()
                                             ...
                                             bool flatten = false, bool overwrite = true)
        {
            ...
            foreach (string url in m_urls)
            {
                if (filteredFiles != null)
                {
                    ...
                    {
                        Wildcard currentFilename = new Wildcard(GetFilename(filteredFile), RegexOptions.IgnoreCase);
                        Wildcard currentExt = new Wildcard(GetExtension(filteredFile), RegexOptions.IgnoreCase);
