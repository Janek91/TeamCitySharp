...
namespace TeamCitySharp.ActionTypes
{
  public class BuildQueue : IBuildQueue
  {
    ...
    {
      BuildWrapper buildWrapper = m_caller.Get<BuildWrapper>($"/app/rest/buildQueue?locator=buildType:({locator})");
    ...
    {
      BuildWrapper buildWrapper = m_caller.Get<BuildWrapper>($"/app/rest/buildQueue?locator=project:({locator})");
      return int.Parse(buildWrapper.Count) > 0 ? buildWrapper.Build : new List<Build>();
    }
  }
}