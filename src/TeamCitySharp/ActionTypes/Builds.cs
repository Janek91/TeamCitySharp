...
namespace TeamCitySharp.ActionTypes
{
    public class Builds : IBuilds
    {
        ...
        {
            Builds newInstance = (Builds)MemberwiseClone();
        ...
        {
            string strParam = GetParamLocator(param);
            BuildWrapper buildWrapper =
        ...
        {
            List<Build> builds = ByBuildLocator(BuildLocator.WithDimensions(BuildTypeLocator.WithId(buildConfigId),
                                                                    status: BuildStatus.SUCCESS,
                                                                    maxResults: 1
                                          ), param);
        ...
        {
            Build build = m_caller.GetFormat<Build>(ActionHelper.CreateFieldUrl("/app/rest/builds/id:{0}", m_fields), id);
        ...
        {
            List<Build> builds = ByUserName(userName);
        ...
        {
            string strIncludeInitial = includeInitial ? "true" : "false";
            if (param == null)
            {
                param = new List<string> { "defaultFilter:false" };
            }
        ...
        {
            string url = $"/downloadBuildLog.html?buildId={projectId}&archived={zipped}";
        ...
        private static string GetParamLocator(List<string> param)
        {
            ...
            if (param != null)
            {
                foreach (string tmpParam in param)
                {
                    strParam += ",";
                    strParam += tmpParam;
                }
